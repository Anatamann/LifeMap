...
export function UserProvider({ children }: { children: React.ReactNode }) {
  ...
  // Calculate entries this week using timezone-aware dates
  const thisWeekEntries = entries.filter(entry => DateUtils.isThisWeek(entry.date));

  // Calculate entries this month using timezone-aware dates
  const thisMonthEntries = entries.filter(entry => DateUtils.isThisMonth(entry.date));
  ...
  const getTodaysEntry = (): JournalEntry | null => {
    const today = DateUtils.getCurrentLocalDate();
